...
class DBHistory(torch.nn.Module):
    ...
    def __init__(self,  num_nodes: int, num_layers:int, embedding_dim: int, multi_db = False, device=None, db_value_from_tensor = None, tensor_from_dbvalue = None, list_from_dbvalue = None):
        super().__init__()
    ...
    def pull(self, n_id: Tensor = None, layer: int= None) -> Tensor:
        ...
        out = torch.Tensor([self.list_from_dbvalue(vv[k],self.embedding_dim) for k in keys])
    ...
    @torch.no_grad()
    def push(self, x, n_id: Tensor = None, layer:int = None, offset: Optional[Tensor] = None, count: Optional[Tensor] = None):
        ...
        if offset is None or count is None:
            if n_id.shape[0] == 1:
                if self.multi_db:
                    self.db[layer].put(self.key(n_id.item(),layer), self.db_value_from_tensor(x),disable_wal=True)
                else:
                    self.db.put(self.key(n_id.item(),layer), self.db_value_from_tensor(x),disable_wal=True)
            else:
                for index,j in enumerate(n_id):
                    self.wbatch.put(self.key(j.item(),layer), self.db_value_from_tensor(x[index]))
                if self.multi_db:
            ...
            for dst_o, c, in zip(offset.tolist(), count.tolist()):
                for i in range(c):
                    self.wbatch.put(self.key(dst_o+i,layer), self.db_value_from_tensor(x[src_o+i]))
                src_o +=c
            if self.multi_db:
                self.db[layer].write(self.wbatch,disable_wal=True)
                self.wbatch.clear()
